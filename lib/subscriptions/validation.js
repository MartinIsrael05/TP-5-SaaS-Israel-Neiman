...
export function validateSubscription(input) {
  ...
  const usageLevelRaw = String(input.usageLevel ?? "").trim();
  const usageLevel = USAGE_LEVELS.includes(usageLevelRaw) ? usageLevelRaw : "Alto";
  const currencyRaw = String(input.currency ?? "").trim().toUpperCase();
  const currency = CURRENCIES.includes(currencyRaw) ? currencyRaw : DEFAULT_CURRENCY;
...
export { BILLING_CYCLES, STATUSES, USAGE_LEVELS };
