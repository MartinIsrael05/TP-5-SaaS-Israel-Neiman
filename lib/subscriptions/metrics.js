...
import { CATEGORIES_FALLBACK_LABEL } from "./constants";
import { resolveNextChargeDate } from "./dates";
...
/**
 * Los numeros grandes de la fila superior del panel. ARS y USD se devuelven
 * por separado: sumarlos directo rompe las metricas.
 */
export function summarize(subscriptions) {
  ...
  const zombies = active.filter((item) => item.usageLevel === "Bajo");

  const isUSD = (subscription) => subscription.currency === "USD";

  const monthlyTotalARS = active
    .filter((subscription) => !isUSD(subscription))
    .reduce((total, subscription) => total + monthlyAmount(subscription), 0);
  const monthlyTotalUSD = active
    .filter(isUSD)
    .reduce((total, subscription) => total + monthlyAmount(subscription), 0);

  // Costo mensual prorateado de las suscripciones activas de poco uso
  // ("zombies"): plata que se sigue pagando por algo que casi no se usa.
  const potentialSavingsARS = zombies
    .filter((subscription) => !isUSD(subscription))
    .reduce((total, subscription) => total + monthlyAmount(subscription), 0);
  const potentialSavingsUSD = zombies
    .filter(isUSD)
    .reduce((total, subscription) => total + monthlyAmount(subscription), 0);
  ...
  return {
    monthlyTotalARS,
    monthlyTotalUSD,
    annualProjectionARS: monthlyTotalARS * 12,
    annualProjectionUSD: monthlyTotalUSD * 12,
...
/**
 * Gasto mensual por categoria, de mayor a menor. Separado por moneda: sumar
 * ARS y USD en la misma barra distorsiona el porcentaje de cada categoria.
 */
export function spendByCategory(subscriptions, categories = []) {
  ...
  function breakdownForCurrency(currency) {
  ...
  return {
    categoryBreakdownARS: breakdownForCurrency("ARS"),
    categoryBreakdownUSD: breakdownForCurrency("USD"),
  };
}
  ...
  for (const subscription of subscriptions) {
    if (subscription.status === "paused") {
      suggestions.push({
        ...
        detail: `Si la cancelas te ahorras ${formatMoneyShort(monthlyAmount(subscription) * 12, subscription.currency)} por año.`,
      ...
      if (chargeDate) {
        ...
        if (daysUntil <= 45) {
          suggestions.push({
            ...
            detail: `Es un cobro anual de ${formatMoneyShort(subscription.amount, subscription.currency)} de una sola vez.`,
            subscriptionId: subscription.id,
          });
        }
      }
